...
/**
 * Check if a location is within the geo-fence (REQUIRED validation)
...
export const checkGeoFence = async (latitude, longitude) => {
  try {
    ...
    if (!settingsDoc.exists()) {
      // If no geo-fence is set, block attendance (fail closed for security)
      return {
        inside: false,
        message: 'Geo-fence not configured. Please contact administrator.',
      }
    }
    ...
    if (!centerLat || !centerLon || !radius) {
      // If settings are incomplete, block attendance (fail closed for security)
      return {
        inside: false,
        message: 'Geo-fence settings incomplete. Please contact administrator.',
      }
    }
    ...
    // On error, block attendance (fail closed for security)
    return {
      inside: false,
      message: 'Error checking geo-fence. Please try again or contact administrator.',
    }
  }
}
